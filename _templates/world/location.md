...
{Room} as room
---
let columns = ["Type", "Plane", "Fragment", "Galaxy", "System", "Planet", "Satellite", "Continent", "Region", "Settlement", "District", "Neighbourhood", "Building", "POI", "Section", "Room"];


let res = `
\`\`\`dataview
TABLE WITHOUT ID link(file.name) as "Location", ${columns.join(", ")}
FROM #world AND "World/World Description"
WHERE Plane = "${context.bound.plane}" AND file.name != ""
